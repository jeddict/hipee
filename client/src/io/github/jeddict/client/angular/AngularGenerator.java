...
import static io.github.jeddict.jcode.util.FileUtil.*;
...
public class AngularGenerator extends BaseWebGenerator {
...
//    }
    ...
    private void addMavenDependencies(Reader pom) {
...
//        String pomPath = applicationConfig.getClientPackageManager() + "_" + (applicationConfig.isUseSass() ? "sass_" : "");
        ...
        try (Reader sourceReader = new InputStreamReader(loadResource(ANGULAR_TEMPLATE + "pom/_pom.xml"))) {
            try (Reader targetReader = new StringReader(parser.parse(sourceReader))) {
                addMavenDependencies(targetReader);
            } catch (ScriptException ex) {
                Exceptions.printStackTrace(ex);
                System.out.println("Error in template : " + ANGULAR_TEMPLATE + "pom/_pom.xml");
            }
        }
    ...
    public void postExecute() {
        ...
        handler.info(NbBundle.getMessage(this.getClass(), "WEBPACK_PROFILE_TITLE"),
                NbBundle.getMessage(this.getClass(), "WEBPACK_PROFILE_TEXT",
                        getAppConfig().getClientPackageManager()));
    }
